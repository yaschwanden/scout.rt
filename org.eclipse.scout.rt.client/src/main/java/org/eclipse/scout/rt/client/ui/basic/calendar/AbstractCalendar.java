...
public abstract class AbstractCalendar extends AbstractWidget implements ICalendar, IContributionOwner, IExtensibleObject {
  ...
  protected void initConfig() {
    ...
    for (Class<? extends ICalendarItemProvider> itemProviderClazz : configuredProducers) {
      try {
        ...
        menus.addAllOrdered(MenuUtility.getMenus(provider.getMenus(), MenuUtility.createMenuFilterMenuTypes(CollectionUtility.hashSet(CalendarMenuType.EmptySpace), false)));
      }
      catch (Exception e) {
  ...
  protected void updateContentProviderMenus(ICalendarItemProvider provider) {
    ...
    if (provider != null) {
      m_inheritedMenusOfSelectedProvider = MenuUtility.getMenus(provider.getMenus(), MenuUtility.createMenuFilterMenuTypes(CollectionUtility.hashSet(CalendarMenuType.CalendarComponent), false));
